...
import { Reservation, AppView, Announcement, MenuItem, CartItem } from './types';
import AdminDashboard from './components/AdminDashboard';
import { fetchReservations, createReservation, updateReservationStatusService, checkConnection, fetchAnnouncements, fetchMenu, updateMenuItemPrice, DEFAULT_MENU_ITEMS, createMenuItem, createOrder, updateOrderStatus } from './services/supabase';
...
const App: React.FC = () => {
  ...
  const [lastReservationDetails, setLastReservationDetails] = useState<any>(null);
  ...
  const addToCart = (item: any) => {
    setCartItems(prev => {
      ...
      if (existing) {
        showToast(`+1 ${item.name} adicionado ao pré-pedido!`, 'success');
        return prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i);
      }
      showToast(`${item.name} adicionado à reserva!`, 'success');
      return [...prev, { ...item, quantity: 1 }];
    });
    // Scroll hint or just feedback
  };
  ...
  const [formData, setFormData] = useState({
    ...
    paymentMethod: 'card' as 'pix' | 'card' | 'money'
  });
  ...
  // --- COMBINED SUBMIT LOGIC (RESERVATION + ORDER) ---
  const handleReservationSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    // Validation
    if (!formData.name || !formData.date || !formData.phone) {
      showToast("Por favor, preencha nome, data e telefone.", "error");
      return;
    }

    try {
      showToast("Confirmando mesa e pedido...", "info");
      ...
      if (cartItems.length > 0) {
         ...
         // Note: We use the same name/phone for the order. Status is 'pending_payment' so it shows on KDS.
  ...
  const updateMenuPrice = async (id: string, newPrice: number) => {
    try {
      // Optimistic Update: Update state directly to prevent swapping the list with default data
      setMenuItems(prev => prev.map(item => item.id === id ? { ...item, price: newPrice } : item));
      
      // Call service to persist to DB/Local
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-200 font-sans selection:bg-orange-600 selection:text-white pb-20">
      ...
      {/* GLOBAL ANNOUNCEMENT STACK */}
      <div className="fixed top-14 left-0 w-full z-30 flex flex-col pointer-events-none">
      ...
      {/* SUCCESS MODAL (RESERVATION + ORDER) */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-stone-900 border border-stone-800 p-8 rounded-2xl shadow-2xl w-full max-w-md relative mx-4">
              <button onClick={() => setShowSuccessModal(false)} className="absolute top-4 right-4 text-stone-500 hover:text-white">
                <X size={20} />
              </button>
              ...
              <div className="text-center py-4">
                  ...
                  <h3 className="text-2xl font-serif font-bold text-white mb-2">Tudo Certo!</h3>
                  <p className="text-stone-400 text-sm mb-6 leading-relaxed">
                     Sua mesa para <strong className="text-white">{lastReservationDetails?.reservation?.pax}</strong> foi reservada para <strong className="text-white">{lastReservationDetails?.reservation?.date}</strong> às <strong className="text-white">{lastReservationDetails?.reservation?.time}</strong>.
                     {lastReservationDetails?.hasOrder && (
                        <span className="block mt-2 text-orange-400">
                           Seu pedido prévio também foi enviado para a cozinha!
                        </span>
                     )}
                  </p>
                  ...
                  <button 
                    ...
                    className="w-full bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-emerald-700 transition-colors"
                  >
                    Maravilha!
                  </button>
              </div>
      ...
      <nav className="fixed w-full z-50 transition-all duration-300 bg-stone-950/80 backdrop-blur-md border-b border-white/5 top-0 h-14">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[0.5rem] uppercase tracking-[0.3em] text-orange-500 font-medium">Churrascaria Premium</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 md:hidden z-50">
             <button onClick={toggleMobileMenu} className="text-white hover:text-orange-500 transition-colors">
        ...
        `}>
           ...
           `}>
               ...
               <button onClick={() => { toggleMobileMenu(); document.getElementById('reservas')?.scrollIntoView({ behavior: 'smooth' }); }} className="w-full bg-orange-600 text-white py-3 rounded-full text-sm font-bold tracking-widest mt-2 hover:bg-orange-700 transition-all shadow-lg shadow-orange-900/20">
                 FAZER RESERVA
               </button>
           </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="mb-12">
            ...
            <div className="w-24 h-1 bg-orange-600"></div>
             <p className="text-stone-400 mt-2">Selecione os pratos que deseja adiantar para sua reserva.</p>
          </div>
              ...
              {filteredItems.map((item) => (
                <div key={item.id} className="group bg-stone-900 border border-stone-800 rounded-2xl overflow-hidden hover:border-orange-900/50 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-orange-900/10">
                  ...
                  <div className="p-5">
                    ...
                    >
                      Adicionar à Reserva <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <div>
              ...
              <p className="text-stone-600 text-lg mb-8 leading-relaxed">
                Garanta sua mesa e desfrute de momentos únicos. Para agilizar seu atendimento, você pode deixar seu pedido pré-escolhido.
              </p>
              ...
              <div className="text-sm text-stone-500">
                <p>O pré-pedido não cobra nada agora. O pagamento é feito no restaurante.</p>
              </div>
            </div>

            {/* Form Side */}
            <div className="bg-white p-6 md:p-10 rounded-2xl md:rounded-[2rem] shadow-2xl border border-stone-200 sticky top-20">
              ...
              <form className="space-y-6" onSubmit={handleReservationSubmit}>
                ...
                <div className="pt-4 border-t border-dashed border-stone-300">
                    <div className="flex items-center gap-2 text-orange-600 font-bold mb-4">
                       <Utensils size={18} />
                       <span>Pré-pedido (Opcional)</span>
                    </div>

                    {cartItems.length === 0 ? (
                       <div className="bg-stone-50 p-4 rounded-lg text-center border border-stone-200">
                          <p className="text-sm text-stone-500">Nenhum prato selecionado.</p>
                          <a href="#menu" className="text-orange-600 font-bold text-xs underline mt-1 inline-block">Ver Cardápio</a>
                       </div>
                    ) : (
                             ...
                             <div key={item.id} className="flex justify-between items-center text-sm">
                                <div className="flex items-center gap-2">
                                   <div className="flex items-center border border-stone-300 rounded bg-white">
                                      <button type="button" onClick={() => updateQuantity(item.id, -1)} className="px-2 py-0.5 text-stone-500 hover:bg-stone-100 rounded-l"><Minus size={10}/></button>
                                      <span className="w-6 text-center text-xs font-bold text-stone-700">{item.quantity}</span>
                                      <button type="button" onClick={() => updateQuantity(item.id, 1)} className="px-2 py-0.5 text-stone-500 hover:bg-stone-100 rounded-r"><Plus size={10}/></button>
                                   </div>
                                   <span className="text-stone-700 line-clamp-1 max-w-[120px]">{item.name}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                   <span className="font-bold text-stone-900">R$ {(item.price * item.quantity).toFixed(2)}</span>
                                   <button type="button" onClick={() => removeFromCart(item.id)} className="text-stone-400 hover:text-red-500"><Trash2 size={14}/></button>
                                </div>
                             </div>
                          ))}
                          <div className="flex justify-between items-center pt-2 border-t border-stone-200">
                             <span className="font-bold text-stone-900">Total Previsto</span>
                ...
                <div className="space-y-2">
                   <label className="text-sm font-semibold text-stone-800">Forma de Pagamento (No Local)</label>
                   <select 
                     className="w-full bg-white border border-stone-300 rounded-lg p-3 text-base text-stone-900 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all"
                ...
                <div className="pt-2">
                  <button type="submit" className="w-full bg-stone-900 text-white font-bold py-4 rounded-xl hover:bg-stone-800 transition-all shadow-xl text-base flex items-center justify-center gap-2">
                    {cartItems.length > 0 ? "CONFIRMAR MESA & PEDIDO" : "CONFIRMAR RESERVA"}
                    <ArrowRight size={18} />
                  </button>
                  <p className="text-center text-xs text-stone-500 mt-4">
                    * Sua mesa está garantida. Pagamento realizado após a refeição.
                  </p>
                </div>
              </form>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16">
            <div className="flex flex-col items-center md:items-start">
              ...
              <p className="text-stone-500 text-sm tracking-widest uppercase">Churrascaria Premium & Massas</p>
            </div>
          ...
          <div className="border-t border-stone-900 pt-8 flex flex-col md:flex-row justify-between items-center text-stone-500 text-xs gap-4 text-center md:text-left">
            <p>&copy; 2024 Fuego Prime. Todos os direitos reservados.</p>
